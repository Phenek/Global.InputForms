    ...
    public class BlankDatePickerRenderer : EntryRenderer
    {
        ...
        private void UpdateDate()
        {
            if (blankPicker.DateSet)
            {
                ...
                if (_picker.Date.ToGlobalDateTime().Date != blankPicker.Date.Date)
                    _picker.SetDate(blankPicker.Date.Date.ToGlobalNSDate(), false);
            }
            else
            {
        ...
        public void SetInputAccessoryView()
        {
            ...
            if (!string.IsNullOrEmpty(blankPicker.DoneButtonText))
            {
                var doneButton = new UIBarButtonItem(blankPicker.DoneButtonText, UIBarButtonItemStyle.Done,
                    (s, ev) =>
                    {
                        blankPicker.Text = Control.Text = _picker.Date.ToGlobalDateTime().Date.ToString(blankPicker.Format);
                        blankPicker.Date = _picker.Date.ToGlobalDateTime().Date;
                        if (blankPicker != null)
        ...
        {
            _picker.MaximumDate = blankPicker.MaximumDate.ToGlobalNSDate();
        }

        private void UpdateMinimumDate()
        {
            _picker.MinimumDate = blankPicker.MinimumDate.ToGlobalNSDate();
        }
